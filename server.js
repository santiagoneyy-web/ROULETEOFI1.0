...
function sseNotify(tableId, data) {
    const clients = sseClients[tableId] || [];
...
app.post('/api/spin', async (req, res) => {
    ...
    try {
        ...
        if (isMongo) {
            ...
            currentHistory.reverse(); // Chronological order
        } else {
    ...
    if (ag5Result) {
        ...
        if (isMongo) {
            ...
            sseNotify(String(table_id), { type: 'spin', number, id: newId, table_id });
            res.json(newSpin);
        } else {
            // Fallback
            db.addSpin(table_id, number, source || 'bot', { event_id: req.body.event_id }, (err, id) => {
                if (err) return res.status(500).json({ error: err.message });
                res.json({ id, table_id, number, source, note: 'Saved to fallback JSON' });
            });
        }
